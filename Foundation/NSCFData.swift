...
internal final class NSCFData : NSMutableData {
    override var length: Int {
        get {
            return CFDataGetLength(_unsafeReferenceCast(self, to: CFData.self))
        }
        set {
            CFDataSetLength(_unsafeReferenceCast(self, to: CFMutableData.self), newValue)
        }
    }
    
    override var bytes: UnsafeRawPointer {
        if let bytes = CFDataGetBytePtr(_unsafeReferenceCast(self, to: CFData.self)) {
    ...
    override var mutableBytes: UnsafeMutableRawPointer {
        return UnsafeMutableRawPointer(CFDataGetMutableBytePtr(_unsafeReferenceCast(self, to: CFMutableData.self)))
    }
...
internal func _CFSwiftDataLength(_ data: CFTypeRef) -> CFIndex {
    return _unsafeReferenceCast(data, to: NSData.self).length
}

internal func _CFSwiftDataBytes(_ data: CFTypeRef) -> UnsafeRawPointer? {
    return _unsafeReferenceCast(data, to: NSData.self).bytes
}

internal func _CFSwiftDataGetBytes(_ data: CFTypeRef, _ buffer: UnsafeMutableRawPointer, _ range: CFRange) {
    _unsafeReferenceCast(data, to: NSData.self).getBytes(buffer, range: NSRange(location: range.location, length: range.length))
}

internal func _CFSwiftMutableDataMutableBytes(_ data: CFTypeRef) -> UnsafeMutablePointer<UInt8>? {
    let d = _unsafeReferenceCast(data, to: NSMutableData.self)
...
internal func _CFSwiftMutableDataSetLength(_ data: CFTypeRef, _ length: CFIndex) {
    _unsafeReferenceCast(data, to: NSMutableData.self).length = length
}

internal func _CFSwiftMutableDataIncreaseLengthBy(_ data: CFTypeRef, _ amt: CFIndex) {
    _unsafeReferenceCast(data, to: NSMutableData.self).increaseLength(by: amt)
}

internal func _CFSwiftMutableDataAppendBytes(_ data: CFTypeRef, _ bytes: UnsafePointer<UInt8>, _ length: CFIndex) {
    _unsafeReferenceCast(data, to: NSMutableData.self).append(bytes, length: length)
}
...
internal func _CFSwiftMutableDataReplaceBytesInRange(_ data: CFTypeRef, _ range: CFRange, _ bytes: UnsafeMutableRawPointer?, _ length: CFIndex) {
    _unsafeReferenceCast(data, to: NSMutableData.self).replaceBytes(in: NSRange(location: range.location, length: range.length), withBytes: bytes, length: length)
}
