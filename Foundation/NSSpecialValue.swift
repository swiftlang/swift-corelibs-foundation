...
internal class NSSpecialValue : NSValue {
    ...
    override func getValue(value: UnsafeMutablePointer<Void>) {
        // XXX unsafe bitcasts to different pointer sizes will fail
        UnsafeMutablePointer<Void>(value).moveInitializeFrom(unsafeBitCast(_value, UnsafeMutablePointer<Void>.self),
                                                             count: sizeofValue(_value.dynamicType))
    }
    ...
    override var description : String {
        if let printable = _value as? CustomStringConvertible {
