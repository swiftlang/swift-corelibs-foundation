...
public class NSProgress : NSObject {
    ...
    */
    public class func current() -> NSProgress? {
        return NSProgress._currentProgress.get { NullableProgress() }.progress
    }

    /* Return an instance of NSProgress that has been initialized with -initWithParent:userInfo:. The initializer is passed the current progress object, if there is one. In many cases you can simply precede code that does a substantial amount of work with an invocation of this method, with repeated invocations of -setCompletedUnitCount: and -isCancelled in the loop that does the work.
     
     You can invoke this method on one thread and then message the returned NSProgress on another thread. For example, you can let the result of invoking this method get captured by a block passed to Dispatch.async(). In that block you can invoke methods like -becomeCurrentWithPendingUnitCount: and -resignCurrent, or -setCompletedUnitCount: and -isCancelled.
     */
    ...
    You can invoke this method on one thread and then message the returned NSProgress on another thread. For example, you can let the result of invoking this method get captured by a block passed to Dispatch.async(). In that block you can invoke methods like -becomeCurrentWithPendingUnitCount: and -resignCurrent, or -setCompletedUnitCount: and -isCancelled.
    */
    convenience public init(totalUnitCount unitCount: Int64) {
        self.init(parent: nil, userInfo: nil)
    ...
    */
    public init(parent parentProgressOrNil: NSProgress?, userInfo userInfoOrNil: [NSObject : AnyObject]? = [:]) {
    ...
    */
    public func becomeCurrent(withPendingUnitCount unitCount: Int64) {
        NSProgress.setCurrent(progress: self)
    }
    ...
    */
    public func resignCurrent() {
        NSProgress.setCurrent(progress: nil)
    }
...
internal extension NSProgress {
    static var _currentProgress = NSThreadSpecific<NullableProgress>()
    class func setCurrent(progress: NSProgress?) {
        NSProgress._currentProgress.get { NullableProgress() }.progress = progress
    }
}
