...
internal class _HTTPURLProtocol: _NativeProtocol {
    ...
    override func validateHeaderComplete(transferSate: _NativeProtocol._TransferState) -> URLResponse? {
        if !transferSate.isHeaderComplete {
           return HTTPURLResponse(url: transferSate.url, statusCode: 200, httpVersion: "HTTP/0.9", headerFields: [:])
