...
fileprivate struct _PlistKeyedEncodingContainer<K : CodingKey> : KeyedEncodingContainerProtocol {
    ...
    public mutating func encodeNil(forKey key: Key)               throws { self.container[key.stringValue._bridgeToObjectiveC()] = _plistNullNSString }
    ...
    public mutating func encode(_ value: Float, forKey key: Key)  throws { self.container[key.stringValue._bridgeToObjectiveC()] = self.encoder.box(value) }
    public mutating func encode(_ value: Double, forKey key: Key) throws { self.container[key.stringValue._bridgeToObjectiveC()] = self.encoder.box(value) }
...
extension _PlistEncoder {
    ...
    fileprivate func box_<T : Encodable>(_ value: T) throws -> NSObject? {
        if T.self == Date.self || T.self == NSDate.self {
            ...
            return _SwiftValue.store(value)
...
extension _PlistDecoder {
    ...
    fileprivate func unbox<T : Decodable>(_ encodedValue: Any, as type: T.Type) throws -> T? {
        let decoded: T

        guard let value = _SwiftValue.fetch(encodedValue as? AnyObject) else { return nil }
