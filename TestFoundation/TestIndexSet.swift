...
class TestIndexSet : XCTestCase {
    ...
    func testShift() {
#if !DEPLOYMENT_RUNTIME_SWIFT // Shifting is unimplemented
        var someIndexes = IndexSet(integersIn: 1..<5)
        someIndexes.insert(integersIn: 8..<11)
        someIndexes.insert(15)
    ...
    func testUnion() {
...
#if !DEPLOYMENT_RUNTIME_SWIFT // this test is failing for some unknown reason
...
#if !DEPLOYMENT_RUNTIME_SWIFT // probably similar failure as before...
        ...
        do {
            is1 = IndexSet(integer: 42)
            is2 = IndexSet(integer: 42)
#if !DEPLOYMENT_RUNTIME_SWIFT // it seems that ranges dont collapse
    ...
    func test_AnyHashableContainingIndexSet() {
        ...
        XCTAssertSameType(IndexSet.self, type(of: anyHashables[0].base))
        XCTAssertSameType(IndexSet.self, type(of: anyHashables[1].base))
        XCTAssertSameType(IndexSet.self, type(of: anyHashables[2].base))
