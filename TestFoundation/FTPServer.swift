...
class _FTPSocket {
    ...
    init(port: UInt16) throws {
        ...
        dataSocket = try attempt("socket", valid: isNotNegative, socket(AF_INET, SOCKSTREAM, Int32(IPPROTO_TCP)))
         var on1: Int = 1
         _ = try attempt("setsockopt", valid: isZero, setsockopt(dataSocket, SOL_SOCKET, SO_REUSEADDR, &on1, socklen_t(MemoryLayout<Int>.size)))
         ...
         try socketAddress1.withMemoryRebound(to: sockaddr.self, capacity: MemoryLayout<sockaddr>.size, {
            ...
            DispatchQueue(label: "delay").async {
                do {
                    self.dataSocket = try self.attempt("accept", valid: self.isNotNegative, accept(self.dataSocket, addr, &sockLen))
    ...
    func readDataOnDataSocket() throws -> String {
        var buffer = [UInt8](repeating: 0, count: 4096)
        _ = try attempt("read", valid: isNotNegative, CInt(read(dataSocket, &buffer, 4096)))
        return String(cString: &buffer)
    }
    ...
    func writeRawData(socket data: Data) throws -> Int32{
        ...
        _ = try data.withUnsafeBytes { ptr in
            bytesWritten = try attempt("write", valid: isNotNegative, CInt(write(dataSocket, ptr, data.count)))
        }
        return bytesWritten
    }
...
class _FTPServer {
    ...
    public func parseHeaderData() throws {
        let saveData =  "FTP Implementation to test FTP download and data tasks.Instead of sending file, we are sending the hardcoded data.We are going to test FTP data,Download aand upload tasks with delegates & completion handlers.Creating the data here as we need to pass the count as part of the header.\r\n ".data(using: String.Encoding.utf8)
        ...
        if(read.contains("anonymous")) {
            ...
            //send Data here through Data Socket
            do {
                let  dataWritten = try respondWithData(with: saveData!)
                if dataWritten != -1 {
                    // send the end header in Command socket
            ...
            /*
             Request is for Upload.As we are only dealing with data,Just read data and ignore
             */
            try respondWithRawData(with: "150 Ok to send data.\r\n")
            //read data from the data socket and respond with completion header once you are done
            do {
