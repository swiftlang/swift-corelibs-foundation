...
//
// RUN: echo "TestCalendar.runAllTests()" >> %t/main.swift
// RUN: %target-build-swift %t/main.swift FoundationSupport/XCTStdlibStub.swift -I %S/Inputs/FoundationBridge/ -Xlinker %t/FoundationBridgeObjC.o -o %t/test.out
...
class TestCalendar : XCTestCase {
    ...
    static var allTests: [(String, (TestCalendar) -> () throws -> Void)] {
        return [
            ...
            ("test_AnyHashableContainingCalendar", test_AnyHashableContainingCalendar),
            ("test_AnyHashableCreatedFromNSCalendar", test_AnyHashableCreatedFromNSCalendar),
        ]
    }
...
#if !DEPLOYMENT_RUNTIME_SWIFT // disabled since the underlying implementation does not yet exist
