...
class TestData : XCTestCase {
    static var allTests: [(String, (TestData) -> () throws -> Void)] {
        let tests: [(String, (TestData) -> () throws -> Void)] = [
            ...
            ("test_classForCoder", test_classForCoder),
            ("test_AnyHashableContainingData", test_AnyHashableContainingData),
            ("test_AnyHashableCreatedFromNSData", test_AnyHashableCreatedFromNSData),
        ]
...
#else
        return tests + [
            ("test_bridgeOverrides", test_bridgeOverrides),
            ("test_basicDataMutation", test_basicDataMutation),
            ("test_basicMutableDataMutation", test_basicMutableDataMutation),
    ...
    class AllOnesData : NSMutableData {
        ...
        convenience init(length : Int) {
            self.init(bytes: nil, length: length)
        }
        
        override init(bytes: UnsafeMutableRawPointer?, length: Int, copy: Bool = false, deallocator: ((UnsafeMutableRawPointer, Int) -> Void)? = nil) {
            _length = length
    ...
    func testBridgingCustom() {
        ...
        let dirPath = NSString(string: NSTemporaryDirectory()).appendingPathComponent(NSUUID().uuidString)
        try! FileManager.default.createDirectory(atPath: dirPath, withIntermediateDirectories: true, attributes: nil)
        let filePath = NSString(string: dirPath).appendingPathComponent("temp_file")
    ...
    func test_writeFailure() {
        ...
        do {
            ...
            XCTAssertTrue(false, "Should not have thrown non NSError")
        }
    ...
    func expectOverride<T: _ObjectiveCBridgeable>(_ convertible: T, _ selector: String) {
        XCTAssertNotEqual(class_getMethodImplementation(T._ObjectiveCType.self, sel_getUid(selector)), class_getMethodImplementation(object_getClass(convertible._bridgeToObjectiveC()), sel_getUid(selector)), "The bridge of \(T.self) should override \(selector)")
    }
    ...
    func test_basicDataMutation() {
        ...
        XCTAssertFalse(object.verifier.wasMutableCopied)
        ...
        XCTAssertTrue(object.verifier.wasMutableCopied)

        let preservedObjectness = (data as NSData) is MutableDataVerifier
        XCTAssertFalse(preservedObjectness)
    }
    ...
    func test_basicMutableDataMutation() {
        ...
        object.verifier.reset()
        XCTAssertTrue(data.count == object.length)
        XCTAssertFalse(object.verifier.wasCopied)
    ...
    func test_roundTrip() {
        ...
        XCTAssertFalse(identityOfData(data))
    }
    ...
    func test_classForCoder() {
        ...
        let d = Data()._bridgeToObjectiveC()
